...
def score_profession_normalized(student, profession):
    ...
    total_score += subj_score_norm * 30
...
def process_student(student, professions):
    ...
    for prof in professions:
        ...
        scored_professions.append({
            "name": prof["name"],
            "score": score,
            "sphere": prof.get("sphere", "Не указана"),
            "subsphere": prof.get("subsphere", "Не указана"),
            "positives": pos,
            "negatives": neg,
            "description": prof["description"],
            "facts": {
                "salary_range": prof.get("salary_range", "Не указано"),
                "growth_prospects": prof.get("growth_prospects", "Не указано")
            }
        })
...
if __name__ == "__main__":
    ...
    student = students[2]  # Замени индекс на нужного студента
    recommendations = process_student(student, professions)

    with open("database/profession_recommendations.json", "w", encoding="utf-8") as f:
        json.dump(recommendations, f, ensure_ascii=False, indent=2)

    print("✅ Рекомендации сохранены в database/profession_recommendations.json")
